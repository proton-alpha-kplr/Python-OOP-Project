...
def create_tree_from_dict(json_dict):
    ...
    mytree.create_node(tag="Product Classes Hierarchy", identifier="racine")
...
def recursive_tree_from_json(tree, json_dict, parent_node_id):
    for key, value in json_dict.items():
        #Si la clé est une subclasse, on refait la fonction récursive mais avec "value" de la subclass comme dictionnaire principal
        if (key == "subclasses"):
        ...
        #on ne met pas d'autre else, car on ne veut rien faire on arrive à une value d'un dictionnaire qui n'est ni une subclasse, ni un dictionnaire (ex les attributs)
